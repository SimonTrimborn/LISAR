EvaluateLossFunction = function(LossFunction, Model, model_res, alpha_vec, gamma_vec, delta_vec, Ydata_eval, Ydata_ofs, store.time2, store.time1) {
  ...
  if (Model == "LISAR.SCAD") {
  ...
  } else if (Model == "LISAR.Elastic.Net") {
    model_res_eval = model_res
    lambdas_store = matrix(NA, 
                           nrow = length(model_res[[1]][[2]][[1]]) * length(model_res[[1]][[2]][[2]]) * length(model_res[[1]][[2]][[3]]) * length(alpha_vec) * length(delta_vec), 
                           ncol = length(model_res[[1]][[2]]))
    ...
    lambdas_select = lambdas_store[tail(which(eval_save == min(eval_save)), 1),]
    names(lambdas_select) = c("lambda1", "lambda2", "lambda3", "alpha", "delta")
    
    delta_model_res = c()
    for (j4 in 1:length(model_res)) {
      delta_model_res = c(delta_model_res, model_res[[j4]][[2]][[5]])
    }
    j4 = which(delta_model_res == lambdas_select[5])
    j1 = which(model_res[[j4]][[2]][[1]] == lambdas_select[1])
    j2 = which(model_res[[j4]][[2]][[2]] == lambdas_select[2])
    j3 = which(model_res[[j4]][[2]][[3]] == lambdas_select[3])
    opt_model = model_res_eval[[j4]][[1]][[j1]][[j2]][[j3]]
  ...
  MSFE = mean((Ydata_ofs[[1]] - Reduce('+',Map('%*%', opt_model, Ydata_ofs[-1])))^2) 
