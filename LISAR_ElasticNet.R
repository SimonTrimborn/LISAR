LISAR_ElasticNet = function(N, TT, Lags, Ydata, lambda1, lambda2, lambda3, delta.elastic.net, eps1, alpha.pen, reshape) {
        ...
        for(j3 in 1:length(lambda3)) {
          ...
          while(k2 < Lags) {
            ...
            while(k1 < N) {
              ...
              b1 = FistaElasticNetK.mixed.pen(B = do.call(cbind, b[b_order_new[1:k2]]), 
                                        Y = Yr, 
                                        Z = do.call(rbind, Yx[b_order_new[1:k2]]), 
                                        gam = reg_para_matrix, eps = eps1, 
                                        delta.elastic.net = delta.elastic.net)
              for (jjj in 1:k2) {
  ...
  lambdas = list(lambda1, lambda2, lambda3, alpha.pen, delta.elastic.net)
